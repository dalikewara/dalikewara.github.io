...
Di MacOS, hardisk external saya ketika dia tidak ke-eject secara benar pasti akan menjadi read-only ketika saya koneksikan lagi. Bahkan kadang malah ga ke detect (unmounted).

Untuk mengatasinya, maka saya perlu repair disk nya. Pertama saya koneksikan disk externalnya, kemudian mencari process id atau `pid` external disk nya dengan perintah:
...
klik `ok` saja. Biasanya kalau external disk saya tidak ke detect, setelah proses kill pid tadi maka external disk akan otomatis ke mount. Hal ini dikarenakan pid yang conflict tadi sudah dihapus, sehingga pid asli dari disk yang sedang connect sekarang bisa dijalankan.
