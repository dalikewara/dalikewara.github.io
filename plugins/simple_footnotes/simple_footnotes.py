...
def parse_for_footnotes(article_or_page_generator):
    ...
    for article in sequence_gen(all_content):
        if u"[ref]" in article._content and u"[/ref]" in article._content:
            ...
            if endnotes:
                dv = dom.createElement(u"div")
                ol = dom.createElement(u"ol")
                dv.setAttribute(u"class", u"simple-footnotes")
                ...
                spn = dom.createElement(u"span")
                spn.setAttribute(u"class", u"simple-footnotes-title")
                spn.appendChild(dom.createTextNode(u'REFERENCES'))
                dv.appendChild(spn)
                dv.appendChild(ol)
                dom.getElementsByTagName(u"body")[0].appendChild(dv)
